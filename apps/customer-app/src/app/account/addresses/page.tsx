...
export default function SavedAddressesPage() {
  ...
  const saveAddress = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editingId) {
        await updateAddress(editingId, cleanAddress);
  ...
  const confirmRemoveAddress = async (address: SavedAddress) => {
  ...
  return (
    <CustomerAuthGuard>
      <div className="min-h-screen bg-[var(--background)] pb-12">
        ...
        <header className="sticky top-0 z-40 border-b border-[#EEF2EF] bg-white/95 shadow-sm backdrop-blur">
          <div className="mx-auto flex max-w-4xl items-center justify-between px-4 py-3.5 sm:px-6">
            ...
            <button
              ...
              className="flex h-10 shrink-0 items-center gap-2 rounded-xl bg-[var(--primary)] px-3.5 text-xs font-black text-white shadow-sm transition hover:opacity-95 active:scale-95"
            >
              <Plus size={16} />
    ...
    >
              ...
              {isMenuOpen && (
                <>
                  <div
                    ...
                    onClick={onCloseMenu}
                  />
                  <div className="absolute right-0 top-9 z-50 w-48 rounded-2xl border border-[var(--border)] bg-white p-1.5 shadow-xl ring-1 ring-black/5">
                    <button
                      type="button"
                      onClick={() => {
                        onCloseMenu();
                    ...
                    <button
                      ...
                      className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-left text-xs font-semibold text-[var(--text-primary)] transition hover:bg-[var(--surface-soft)]"
                    >
                      <Plus size={15} className="text-[var(--text-muted)]" />
                      Add New Address
                    </button>
      ...
      <div className="mt-4 flex items-center justify-between border-t border-[var(--border)] pt-3">
        <button
          type="button"
          onClick={onEdit}
          className="flex items-center gap-1.5 text-xs font-bold text-[var(--primary)] transition hover:underline"
        >
          <Pencil size={13} />
          Edit Address
        </button>
        ...
        <button
          type="button"
          onClick={onRemove}
          className="flex items-center gap-1.5 text-xs font-bold text-[var(--text-muted)] transition hover:text-[var(--danger)]"
        >
          <Trash2 size={13} />
          Delete
        </button>
