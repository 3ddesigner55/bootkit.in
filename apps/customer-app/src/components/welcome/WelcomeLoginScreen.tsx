...
import CustomerLoginBottomSheet from "@/components/auth/CustomerLoginBottomSheet";
...
export default function WelcomeLoginScreen({ onSkip }: { onSkip?: () => void }) {
    ...
    <section className="fixed inset-0 z-[120] flex min-h-[100dvh] flex-col bg-[var(--background)] lg:hidden">
      ...
      <div className="absolute right-0 top-0 z-10 flex items-center justify-end px-4 py-4">
        <button
          type="button"
          onClick={() => {
            localStorage.setItem("bootkit_welcome_skipped", "true");
            if (onSkip) {
              onSkip();
            } else {
              setVisible(false);
      ...
      <div className="relative mt-auto rounded-t-3xl bg-white px-4 pb-[calc(1.5rem+env(safe-area-inset-bottom))] pt-6">
        <div className="block text-xs text-center font-bold text-[var(--text-primary)]">
        ...
        <button
          ...
          className="mt-5 h-12 w-full rounded-xl bg-[var(--primary)] text-sm font-black text-white disabled:opacity-50 disabled:cursor-not-allowed"
        >
          Continue
        </button>
      ...
      <CustomerLoginBottomSheet
        open={loginSheetOpen}
        initialPhone={phone}
        onClose={() => setLoginSheetOpen(false)}
        onAuthenticated={() => setVisible(false)}
      />
    </section>
  );
}
