...
import { forgotPasswordWithBackend } from "@/services/auth.service";
...
export default function ForgotPasswordPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    const result = await forgotPasswordWithBackend(email.trim());
    if (!result.success) return setError(result.message);
    setMessage("अगर यह email registered है, तो reset link आपके ईमेल पर भेज दिया गया है। कृपया अपना inbox (और spam) चेक करें।");
  };
