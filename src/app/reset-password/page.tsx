...
import { resetPasswordWithBackend } from "@/services/auth.service";
...
function ResetPasswordForm() {
  ...
  useEffect(() => {
    if (!code) {
      setError("Reset link invalid या expire हो गया है। नया link भेजें।");
    } else {
      setReady(true);
    }
  }, [code]);
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (password.length < 8) return setError("Password कम से कम 8 characters का होना चाहिए।");
    if (password !== confirmPassword) return setError("Passwords match नहीं करते।");
    if (!code) return setError("Reset link invalid या expire हो गया है। नया link भेजें।");

    const result = await resetPasswordWithBackend(code, password);
