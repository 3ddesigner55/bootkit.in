...
function ResetPasswordForm() {
  ...
  useEffect(() => { if (!supabase) return; const code = searchParams.get("code"); const restore = async () => { if (code) await supabase.auth.exchangeCodeForSession(code); const { data } = await supabase.auth.getSession(); if (!data.session) setError("Reset link invalid या expire हो गया है। नया link भेजें।"); else setReady(true); }; void restore(); }, [searchParams]);
  const submit = async (event: FormEvent) => { event.preventDefault(); if (password.length < 8) return setError("Password कम से कम 8 characters का होना चाहिए।"); if (password !== confirmPassword) return setError("Passwords match नहीं करते।"); if (!supabase) return setError("Supabase connection configured नहीं है।"); const { error } = await supabase.auth.updateUser({ password }); if (error) return setError(error.message); router.replace("/login"); };
