...
export default function HomeCatalog() {
  ...
  return <section className="pb-7 pt-4 sm:pb-9"><Container>
    ...
    {selectedCategory === "all" && <><div className="mt-4 flex items-end justify-between"><div><p className="text-[10px] uppercase tracking-[.13em] text-[var(--primary)]">Shop by group</p><h2 className="mt-1 text-xl tracking-[-.04em]">Bestsellers</h2></div></div><div className="mt-3 grid grid-cols-3 gap-2 sm:gap-3 lg:grid-cols-6">{productGroups.map((group) => { const thumbs = group.id === "bestsellers" ? activeProducts.filter((product) => product.bestseller).slice(0, 4) : activeProducts.filter((product) => group.slugs.includes(product.categorySlug)).slice(0, 4); return <button key={group.id} type="button" onClick={() => setSelectedCategory(`group:${group.id}`)} className="rounded-2xl border border-[var(--border)] bg-[#f6f7fb] p-2 text-left transition hover:-translate-y-0.5 hover:shadow-[var(--shadow-sm)]"><div className="grid grid-cols-2 gap-1">{Array.from({ length: 4 }).map((_, index) => <span key={index} className="flex aspect-square items-center justify-center rounded-lg bg-white text-lg">{thumbs[index]?.fallbackIcon ?? group.icon}</span>)}</div><p className="mt-2 line-clamp-2 text-center text-[11px] leading-4">{group.title}</p></button>;})}</div></>}
