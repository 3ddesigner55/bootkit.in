...
export default function SearchBar({
  className,
  placeholder = "Search for milk, bread, fruits, vegetables...",
  compact = false,
}: SearchBarProps) {
  ...
  const startVoiceSearch = () => {
    type SpeechWindow = Window & { webkitSpeechRecognition?: new () => { lang: string; start: () => void; onresult: (event: { results: ArrayLike<ArrayLike<{ transcript: string }>> }) => void; onend: () => void; onerror: () => void; } };
    const Recognition = (window as SpeechWindow).webkitSpeechRecognition;
    if (!Recognition) return;
    const recognition = new Recognition(); recognition.lang = "en-IN"; recognition.onresult = (event) => setQuery(event.results[0][0].transcript); recognition.onend = () => setListening(false); recognition.onerror = () => setListening(false); setListening(true); recognition.start();
  };
  ...
  return (
    ...
    >
      ...
      <button type="button" onClick={startVoiceSearch} aria-label="Search with voice" className={`mr-2 flex h-8 w-8 shrink-0 items-center justify-center rounded-full transition ${listening ? "bg-[var(--primary-light)] text-[var(--primary)]" : "text-[var(--text-muted)] hover:bg-[var(--surface-soft)]"}`}><Mic size={18} /></button>

      {query.trim() && (
        <div className="absolute left-0 top-[calc(100%+8px)] z-[80] w-full overflow-hidden rounded-full border border-[var(--border)] bg-white shadow-[var(--shadow-lg)]">
